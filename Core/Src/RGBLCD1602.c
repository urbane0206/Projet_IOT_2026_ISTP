...
void DFRobot_RGBLCD1602_init(RGBLCD1602_t *lcd)
{
	if (lcd->rgbAddr == (0x60)) {
		...
		lcd->REG_BLUE  = 0x02;
		lcd->REG_ONLY  = 0x02;
	} else if (lcd->rgbAddr == (0x60 >> 1)) { // 0x30
...
{
	DFRobot_RGBLCD1602_command(lcd, LCD_CLEARDISPLAY);   // clear display, cursor to 0
	HAL_Delay(2);                                        // this command takes a long time!
}

void DFRobot_RGBLCD1602_home(RGBLCD1602_t *lcd)
{
	DFRobot_RGBLCD1602_command(lcd, LCD_RETURNHOME);     // cursor to 0
	HAL_Delay(2);                                        // this command takes a long time!
}
...
/**
 * @brief Réglage bas niveau du rétroéclairage (R, G, B séparés).
 */
...
/**
 * @brief Règle la couleur du rétroéclairage avec un code 0xRRGGBB.
 *        Ex : RGBLCD1602_setColor(&lcd, LCD_COLOR_GREEN);
 *             RGBLCD1602_setColor(&lcd, 0x20A0FF);
 */
void RGBLCD1602_setColor(RGBLCD1602_t *lcd, uint32_t rgb)
{
	DFRobot_RGBLCD1602_setRGB(lcd,
	                          (rgb >> 16) & 0xFF,   /* R */
	                          (rgb >> 8)  & 0xFF,   /* G */
	                           rgb        & 0xFF);  /* B */
}
...
{
	RGBLCD1602_setColor(lcd, LCD_COLOR_OFF);
}

void DFRobot_RGBLCD1602_setColorWhite(RGBLCD1602_t *lcd)
{
	RGBLCD1602_setColor(lcd, LCD_COLOR_WHITE);
}

void DFRobot_RGBLCD1602_setBacklight(RGBLCD1602_t *lcd, bool on)
{
	RGBLCD1602_setColor(lcd, on ? LCD_COLOR_WHITE : LCD_COLOR_OFF);
}
...
void DFRobot_RGBLCD1602_begin(RGBLCD1602_t *lcd, uint8_t rows, uint8_t charSize)
{
	...
	if (lcd->rgbAddr == (0xC0 >> 1)) {
		...
		///< set MODE2 values : 0010 0000 -> 0x20 (DMBLNK to 1, ie blinky mode)
...
/**
 * @brief Init complète de l'écran + couleur de départ (0xRRGGBB).
 *        Ex : RGBLCD1602_ECRAN_I2C_Init(&lcd, &hi2c1, LCD_COLOR_CYAN);
 */
void RGBLCD1602_ECRAN_I2C_Init(RGBLCD1602_t *lcd, I2C_HandleTypeDef *hi2c, uint32_t rgb)
{
	...
	RGBLCD1602_setColor(lcd, rgb);

	HAL_Delay(10);
}

/**
 * @brief Allume la couleur donnée si activation != 0, sinon éteint le rétroéclairage.
 */
void RGBLCD1602_setColor_control(RGBLCD1602_t *lcd, uint8_t activation, uint32_t rgb)
{
	RGBLCD1602_setColor(lcd, activation ? rgb : LCD_COLOR_OFF);
}
